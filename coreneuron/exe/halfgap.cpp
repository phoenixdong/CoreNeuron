...
namespace coreneuron {
    ...
    HalfGap_Store HalfGap_global;
    nrn_pragma_acc(declare create (HalfGap_global))
    nrn_pragma_omp(end declare target)


    /** all mechanism instance variables */
    struct HalfGap_Instance  {
        const double* __restrict__ g;
        double* __restrict__ vgap;
    ...
    static inline int get_mech_type() {
        return HalfGap_global.mech_type;
    }
    ...
    static inline void setup_global_variables()  {
        ...
        nrn_pragma_acc(update device (HalfGap_global))
        nrn_pragma_omp(target update to(HalfGap_global))
    ...
    static inline void setup_instance(NrnThread* nt, Memb_list* ml)  {
        HalfGap_Instance* inst = (HalfGap_Instance*) mem_alloc(1, sizeof(HalfGap_Instance));
        int pnodecount = ml->_nodecount_padded;
        Datum* indexes = ml->pdata;
        inst->g = nt->compute_gpu ? cnrn_target_deviceptr(ml->data+0*pnodecount) : ml->data+0*pnodecount;
        inst->vgap = nt->compute_gpu ? cnrn_target_deviceptr(ml->data+1*pnodecount) : ml->data+1*pnodecount;
    ...
    static inline void cleanup_instance(Memb_list* ml)  {
        HalfGap_Instance* inst = (HalfGap_Instance*) ml->instance;
        mem_free((void*)inst);
    }


    static void nrn_alloc_HalfGap(double* data, Datum* indexes, int type)  {
        // do nothing
    }


    void nrn_constructor_HalfGap(NrnThread* nt, Memb_list* ml, int type) {
        ...
        HalfGap_Instance* __restrict__ inst = (HalfGap_Instance*) ml->instance;

        #endif
    }


    void nrn_destructor_HalfGap(NrnThread* nt, Memb_list* ml, int type) {
        #ifndef CORENEURON_BUILD
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* __restrict__ node_index = ml->nodeindices;
    ...
    /** initialize channel */
    void nrn_init_HalfGap(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml, HalfGap_global) if(nt->compute_gpu))
        {
            int nodecount = ml->nodecount;
            int pnodecount = ml->_nodecount_padded;
            const int* __restrict__ node_index = ml->nodeindices;
    ...
    static inline double nrn_current(int id, int pnodecount, HalfGap_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        double current = 0.0;
        inst->i[id] = (inst->vgap[id] - v) * inst->g[id];
        current += inst->i[id];
        return current;
    }
    ...
    /** update current */
    void nrn_cur_HalfGap(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml, HalfGap_global) if(nt->compute_gpu))
        {
            int nodecount = ml->nodecount;
            int pnodecount = ml->_nodecount_padded;
    ...
    void _halfgap_reg()  {
        ...
        int mech_type = nrn_get_mechtype("HalfGap");
        HalfGap_global.mech_type = mech_type;
        if (mech_type == -1) {
            return;
        }

        _nrn_layout_reg(mech_type, 0);
        point_register_mech(mechanism, nrn_alloc_HalfGap, nrn_cur_HalfGap, NULL, NULL, nrn_init_HalfGap, first_pointer_var_index(), NULL, NULL, 1);
